...
import { IsString, IsNumberString, IsBooleanString, IsDateString, IsArray, IsObject, validate } from 'class-validator';
...
export async function DtoFactory(dtoName: string, data: Record<string, any>[]) {
  const entities = [];
  for (const entity of data) {
    const targetDto = eval(`new ${dtoName}`);

    for (const key in entity) {
      targetDto[key] = entity[key];
    }
    const errors = await validate(targetDto);
    if (errors.length > 0) {
      throw errors.flatMap((error) => Object.values(error.constraints));
    }
    entities.push(targetDto);
  }
  return entities;
}
